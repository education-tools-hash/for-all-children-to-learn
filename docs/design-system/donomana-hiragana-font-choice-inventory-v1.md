...
| v1.0 + Addendum 4 | 2026-08-30 | Phase T6.5-A3''。T6.5-A4横断確認で発見した、`nazori-app.html`の保存済み字体設定値に許可リスト検証がない（`nazorin-print.html`にはA2で実装済み）という設計不整合をParity修正。`#guideFontBtns`の実際の`data-font`値3つをSource of Truthとして許可リストを構築し、未知値・invalid value・`javascript:alert(1)`のような文字列を確実に標準（`BIZ UDPGothic`）へfallbackするよう変更。Security実害（script実行・クラッシュ）は元々なかったことをA4で確認済みのため、本修正はdeterministic fallback / implementation parityが目的。Cross-App Isolation・Activity Record・PDF・3字体、いずれも回帰なし。 |
