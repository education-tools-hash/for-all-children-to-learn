...
| matching-app | ✅ | ✅ | 🟡**訂正(WCAG-JIS-FINDING-INITIAL-RESTORE-1)**: how-ov/settings-ov/record-ovはPattern1。vs-result-ov/clear-ov/edit-ovは「初期focus自体が無い」という旧記載は誤りで、実際はPattern3(`btn-vs-again`/`btn-again`/`edit-name`へ`.focus()`)で実装済み | ✅(端点循環) | ✅**Fix済み・Production反映済み(commit 6e9ddb6、main HEAD `64e026b`、WCAG-JIS-FIX-FAMILY-J-1-RELEASE)** | ✅ **参照実装** | ✅ | 🟡**訂正・FIXED IN RC(WCAG-JIS-FIX-FAMILY-D-RESTORE-1、Production未反映)**: edit-ovは元々PASS。clear-ov/vs-result-ovは実機再検証の結果、旧想定(「ボタン押下は画面遷移するため対象外」)に反しbtn-again/btn-clear-back/btn-vs-again/btn-vs-back/Escapeの**全close経路でBODY退行することを新たに確認**(startGame()/backToSel()がfocusを設定しないため)。card-grid先頭カード/start-btn/btn-backへの復帰をRC実装しRegression PASS(commit未定、branch: fix/family-d-focus-restoration) | ✅(系統A) **参照実装** | A11yパネル例外・背景抑制は引き続き参照実装水準。詳細はfinding-initial-restore-1.md参照 |
...
| okane-app | ✅ | ✅ | ✅(Pattern1) | ✅(端点循環) | ✅**Fix済み・Production反映済み(commit 6e9ddb6、main HEAD `64e026b`、WCAG-JIS-FIX-FAMILY-J-1-RELEASE)。help/settings/records/customの4モーダル共通ハンドラで一括修正済み** | ✅ **参照実装** | ✅ | 🟡**FIXED IN RC(WCAG-JIS-FIX-FAMILY-D-RESTORE-1、Production未反映)**: recordsModalは元々PASS(`recordsModalOpener`)。help/settings/customは実機確認でBODY退行を再確認のうえ、`helpModalOpener`/`customModalOpener`(同型パターン)、settingsは共通A11yパネルProxy構造上の理由で固定`donomanaA11yBtn`復帰をRC実装。customModalは実trigger(`.add-item-card`、tabindexなし)がfocus不能なため`#shop .diff-btn.active`へのfallbackも追加。Regression PASS(branch: fix/family-d-focus-restoration) | ✅(系統A) **参照実装** | A11yパネル例外・背景抑制は引き続き参照実装水準。詳細はfinding-initial-restore-1.md参照 |
...
| schedule-app | ✅ | ✅ | ✅(Pattern1) | ✅(端点循環) | ✅**Fix済み・Production反映済み(commit 6e9ddb6、main HEAD `64e026b`、WCAG-JIS-FIX-FAMILY-J-1-RELEASE)。print-modal/img-modalの2モーダルで確認(new-modalは初期focus自体が無く別問題、対象外)。独自の防御分岐[`modal.contains(active)`]は本失敗モードには無効だったが、境界修正で解消** | ✅ **参照実装** | 🟡**FIXED IN RC(WCAG-JIS-FIX-FAMILY-D-RESTORE-1、Production未反映、print-modalのみ対象)**: `printModalOpener`+`closePrintModal()`をEscape/close button/背景クリック/印刷完了の全close経路に適用、Regression PASS(branch: fix/family-d-focus-restoration)。new-modal/img-modalは同一の欠如パターンの疑いがあるが本Fix対象外、別途確認要 | — (未検証、new-modal/img-modalは対象外) | 🟡(系統C、inertなし) | 背景inertなしでも端点循環が機能する例 |
