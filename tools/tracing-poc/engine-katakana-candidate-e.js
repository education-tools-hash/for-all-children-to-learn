...
(function (root) {
  ...
  // Phase T3-C' Candidate E: reflection-invariant "character-relative
  // stroke extent" tie-breaker. Unlike Candidate C/D (T3-C, rejected —
  // absolute centroid/endpoint position, which reflection moves and thus
  // corrupts, causing Mirror Self-Confusion regressions), a stroke's own
  // bbox-diagonal and arc-length are EXACT isometric invariants: reflecting
  // a character does not change any stroke's width, height, diagonal, or
  // arc length. Normalizing each stroke's extent by the character's own
  // extent yields a scale-relative, reflection-invariant "how big is this
  // stroke relative to the whole character" fingerprint, generalizable to
  // any character (not U-specific).
  //
  // Applied ONLY as a soft tie-breaker when the shape-cost best/second-best
  // permutation gap is below AMBIGUITY_THRESHOLD (calibrated from
  // analyze-ambiguity-distribution.js's measured distribution across all
  // 46 characters — chosen to cover every currently-known near-tie
  // failure case, not picked arbitrarily), so it cannot silently overrule
  // a clearly-decisive shape match anywhere else.
  const EXTENT_TIE_WEIGHT = 0.30;
  const AMBIGUITY_THRESHOLD = 0.005;
  function strokeDiag(feat) { return Math.hypot(feat.bbox.width, feat.bbox.height); }
  function extentCost(userFeat, refFeat, userCharDiag, refCharDiag) {
    const uRatio = userCharDiag > 1e-6 ? strokeDiag(userFeat) / userCharDiag : 0;
    const rRatio = refCharDiag > 1e-6 ? strokeDiag(refFeat) / refCharDiag : 0;
    const uLenRatio = userCharDiag > 1e-6 ? userFeat.length / userCharDiag : 0;
    const rLenRatio = refCharDiag > 1e-6 ? refFeat.length / refCharDiag : 0;
    return Math.abs(uRatio - rRatio) + Math.abs(uLenRatio - rLenRatio);
  }

  // Returns { assignment: [refIndexForUser0, refIndexForUser1, ...], totalCost, costMatrix }
  function matchStrokes(userFeatures, refFeatures, userCharDiag, refCharDiag) {
    ...
    if (n >= 2 && userCharDiag != null && refCharDiag != null) {
      // Determine if shape-only leaves the top-2 permutations in a near-tie.
      const idx0 = refFeatures.map((_, i) => i);
      const shapeCosts = permutations(idx0).map((perm) => {
        let cost = 0;
        for (let i = 0; i < n; i++) cost += costMatrix[i][perm[i]];
        return { perm, cost };
      }).sort((a, b) => a.cost - b.cost);
      const gap = shapeCosts.length > 1 ? shapeCosts[1].cost - shapeCosts[0].cost : Infinity;
      if (gap < AMBIGUITY_THRESHOLD) {
        const extMatrix = userFeatures.map((uf) => refFeatures.map((rf) => extentCost(uf, rf, userCharDiag, refCharDiag)));
        const indices = refFeatures.map((_, i) => i);
        let best = null, bestCost = Infinity;
        permutations(indices).forEach((perm) => {
          let cost = 0;
          for (let i = 0; i < n; i++) cost += costMatrix[i][perm[i]] + EXTENT_TIE_WEIGHT * extMatrix[i][perm[i]];
  ...
  function evaluateCharacter(userStrokes, referenceStrokeDefs, opts) {
    ...
    const userAllPtsForAssignment = userFeatures.flatMap((f) => f.absPoints);
    const userBBoxForAssignment = computeBBox(userAllPtsForAssignment);
    const userDiagForAssignment = Math.hypot(userBBoxForAssignment.width, userBBoxForAssignment.height);
    const refDiagForAssignment = Math.hypot(charBBox.width, charBBox.height);
    const { assignment, costMatrix } = matchStrokes(userFeatures, refFeatures, userDiagForAssignment, refDiagForAssignment);
