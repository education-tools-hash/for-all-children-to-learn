...
- A Repository Ruleset ("Protect main production", `enforcement: active`) has been set on `main` since 2026-09-19 (confirmed via the GitHub API: `GET /repos/.../rulesets/23694921`): it blocks **branch deletion** and **non-fast-forward pushes (force-push)** to `main`. It does **not** require a pull request, does **not** require any status checks, and does **not** block an ordinary fast-forward push from any actor with write access — a write-permission push straight to `main` still goes through technically. Do not describe `main` as "fully protected" or "cannot be pushed to directly": the rules in this file, not GitHub's ruleset, are what stop that.
