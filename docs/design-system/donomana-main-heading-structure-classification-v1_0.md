...
- **scratch-app**: canvas中心のお絵かきツール、`#root`(canvas)+複数パネル → **AUDIT-35-FIX-3I-SCRATCHで実装済み(下記)**
- **gaze-keyboard**: AAC(拡大代替コミュニケーション)ボード、パネル多数・fullscreen専用領域あり → **AUDIT-35-FIX-3J-GAZE-KEYBOARDで実装済み(下記)。Group C全4件が完了**
- **drawing-app**: canvas+sidebar+toolbar群、Gaze設定パネルも混在。Group A候補としたが精査の結果Cへ再分類が妥当 → **AUDIT-35-FIX-3H-DRAWINGで実装済み(下記)**

**AUDIT-35-FIX-3J-GAZE-KEYBOARD実装結果**: `body{display:flex;flex-direction:column;...}`に加え、本アプリ固有の全画面モード専用CSSとして`body.is-fullscreen`が縦2ペイン分割される設計(コード内コメントに明記:「body を縦2ペインに分割 / ペイン1(fs-top): テキスト表示+アクション / ペイン2(keyboard-panel): キーボード(flex:1)」)であることを確認した。`body.is-fullscreen .fs-top{flex-shrink:0}`(ペイン1)と`body.is-fullscreen .keyboard-panel{flex:1 1 0}`(ペイン2)が同一のflexコンテナ(body)の直接の子であることに依存しているため、drawing-appよりもさらに制約が強く、`.keyboard-panel`を他要素と束ねて新規`<main>`でwrapする方式は採らず、`.keyboard-panel`単体をそのまま`<main>`へタグ変換した(Priority A、DOM再配置なし、diffはopen/close2行+h1 1行の計3行)。`#fsTop`(直前の兄弟要素)・`.app-header`/`.profile-bar`/`.text-panel`/`.qbar`/`.phrase-panel`/`.aac-panel`/`.stamp-panel`/`#rewardCanvas`/`.pred-panel`および各種modal(`#settingsModal`/`#phraseModal`/`#profileModal`/`#hrModal`/`#printArea`/`#helpModal`)/`#toast`はすべてmain外を維持。h1は`.app-header`内の既存可視タイトル`.app-title`(「視線キーボード」)をタグ変換、`font-weight:900`指定済み・グローバル`margin:0`resetありでUAデフォルト太字化リスクなし。Gaze dwell機構(`addDwell`/`startDwell`/`cancelDwell`/`finishDwell`、`pointerenter`/`pointerleave`/`pointercancel`ベース)およびSwitch Scanの`buildScanItems()`(`modeTabs`/`keyboardArea`/`phraseGrid`の要素参照+class selectorベース)は、いずれも要素参照・DOM階層非依存の実装であることをコードから確認し、実測でも通常モード(`scanItems.length`=79)・設定モーダル表示中(同83)ともに前後で完全一致することを確認した。通常モード・全画面モード双方のgeometryを実測し、全画面モードの2ペイン分割(`#fsTop`固定高231.2px+`.keyboard-panel`可変`flex-grow:1`、実測高612.8px@390×844)が変更前後で完全一致することを確認した。実際のPlaywright操作で、文字入力(キータップ→テキスト反映)・モードタブ切替(ひらがな⇄カタカナ)・フレーズ/AACボタン・読み上げ(`#btnSpeak`→`speechSynthesis.speaking`確認)・設定モーダル(`#donomanaA11yBtn`→`#donomanaSettingsProxy`の正規導線)・全画面ON/OFFのいずれも正常動作し、console/pageerrorとも0件だった。`tools/settings-proxy-focus-audit/audit.py`は135/135件成功(gaze-keyboard分5件含む)で回帰なし。本Phaseの完了によりmain missing 1→0(全35アプリでmain欠落解消)、h1 missing 3→2(register-app/schedule-appのみ、いずれも可変・ユーザー編集値がタイトルであるため意図的に本Phaseの対象外)となり、「Final AUDIT-35構造Gate」(main missing=0、h1 excess=0、h1 missing=2件のみ)を達成した。

**AUDIT-35-FIX-3I-SCRATCH実装結果**: drawing-appと異なり、`body`自体は`display:flex`ではなく(`html,body{height:100%;overflow:hidden}`のみ)、既存の単一wrapper`#root`が`display:flex;flex-direction:column;width:100dvw;height:100dvh;position:relative`で自己完結的にviewportサイズを持つ設計だった(bodyのflex子であることに依存しない)。そのため`#root`(既にheader/h1/toolbar/canvas-wrapper(`#uc`+`#sc`の2枚canvas)/progress/footerを内包)をそのまま`<main>`へタグ変換するだけで済み、drawing-appのような境界縮小の判断は不要だった(Priority A、diffはopen/close2行のみ)。既存h1(「🎨 けずりえ」、`#root`内`#hdr`にすでに存在)は無変更のまま維持。`<canvas id="cfc">`(完成判定用の非表示canvas)と各種modal(`#cov`/`#setOv`/`#helpOv`/`#txtEdOv`、いずれも`role="dialog" aria-modal="true"`)・`#ttsBbl`は`#root`の外にあり、そのままmain外を維持。座標変換は`#cw`(canvas wrapper)の`getBoundingClientRect()`+`sc.width/rect.width`のスケール補正ベースで、DOM階層非依存であることをコードから確認し、実測で3点(中央・左上寄り・右下寄り)の論理座標が完全一致することを確認した。実際にPlaywrightで削り(scratch)ジェスチャーを行い、canvas pixelが実際に変化することも確認済み(drawing-appのheadless制約と異なり、本アプリでは合成pointerイベントでも描画が正しく反映された)。
...
| v1.0(改訂7) | 2026-09-06 | Phase AUDIT-35-FIX-3I-SCRATCH。Group C 4件のうちscratch-appを実装(残るgaze-keyboardのみ個別Phase継続)。`body`は`display:flex`ではなく、既存単一wrapper`#root`自体が`width:100dvw;height:100dvh`で自己完結的にサイズを持つ設計であることを確認し、`#root`(既にh1・toolbar・canvas・footerを内包)をそのまま`<main>`へタグ変換(Priority A、diff2行のみ、h1は無変更)。main missing 2→1、h1 missing不変(3、scratch-appはh1既存のため対象外)。座標変換(`getBoundingClientRect`+スケール補正)の完全一致を実測確認、実際の削りジェスチャーでcanvas pixelが変化することも確認した。 |
| v1.0(改訂8) | 2026-09-06 | Phase AUDIT-35-FIX-3J-GAZE-KEYBOARD。Group C最終4件目のgaze-keyboardを実装し、Group C全4件(sst-app/drawing-app/scratch-app/gaze-keyboard)が完了。`body{display:flex;flex-direction:column}`に加え、全画面モード専用の`body.is-fullscreen`縦2ペイン分割(ペイン1`.fs-top`(`flex-shrink:0`)+ペイン2`.keyboard-panel`(`flex:1 1 0`))がbodyの直接の子であることに依存する設計であることを確認し、複数要素を束ねる新規wrapper方式ではなく、`.keyboard-panel`単体をPriority Aでmain化(位置・階層とも無変更のタグ変換のみ)する最も安全側の設計を採用した。`.app-title`をh1化。main missing 1→0(全35アプリでmain欠落解消、Final AUDIT-35構造Gate達成)、h1 missing 3→2(register-app/schedule-appのみ残存、両者とも可変・ユーザー編集値がタイトルであるため本Phaseでは意図的に対象外、別Phaseで個別設計予定)。CSS/JSともに`.keyboard-panel`へのタグ修飾セレクタ・tagName/instanceof依存が存在しないことをコード確認し、通常モード・全画面モード双方のgeometryが実測で完全一致(`#fsTop`固定高231.2px/`.keyboard-panel`可変`flex-grow:1`の2ペイン分割を含む)することを確認した。Gaze dwell(`addDwell`系)・Switch Scan(`buildScanItems()`)ともに要素参照・class selectorベースでDOM階層非依存であることを確認し、実際の文字入力・モード切替・AAC/フレーズ選択・読み上げ(`#btnSpeak`)・設定モーダル(`#donomanaA11yBtn`→`#donomanaSettingsProxy`経由)・全画面ON/OFFいずれも実インタラクションで正常動作を確認した(console/pageerror 0件)。settings-proxy-focus-audit(135/135)も回帰なし。 |
