...
|---|---|---|---|---|---|---|---|---|---|---|
| register-app | ✅ | ✅ | 🟡(Pattern3、最初のcontrol) | ✅(端点循環) | ✅(Pattern3のためinitial focus=firstと一致、構造的に非該当) | ❌(TIER1-F2) | ✅ | 🟡**FIXED IN RC / USER REVIEW PENDING(WCAG-JIS-FIX-FAMILY-D-REGISTER-1、branch: fix/family-d-register-focus-restoration、Production未反映)**: product-modal(TIER1-F5)。Root Cause = 保存(Save)経路のみ`closeModal()`直前に`renderProducts()`が商品グリッド全体を再生成し、`pmOpenerEl`が指す旧DOMノードがdisconnectedになりBODY退行(cancel/Escapeは元々正常だった、実機LEVEL-A確認)。`pmOpenerProductId`を追加保持し、disconnected時は再描画後の同一商品の`.product-card`(新規追加時は`.add-product-card`)へfallback。Browser ValidationでLEVEL-A確認済み。**別Finding(本Fix対象外)**: delete-modalも同型のBODY退行を新規確認(cancel経路、実機LEVEL-A) | 🟡(系統B、モーダル自身のみ) | 8モーダル共通ヘルパー、Fix影響範囲大。詳細はfamily-d-focus-restoration-close-1.md参照 |
...
| FAMILY-D | Focus Restoration | **1(register-app product-modal、TIER1-F5「pmOpenerEl退行」)。[2026-09-08追記] WCAG-JIS-FIX-FAMILY-D-REGISTER-1でRC Fix実装済み、Production未反映のためOpenのまま維持。matching-app・okane-app・schedule-app・scratch-app・cup_game[settingsOverlay]は全てProduction解消済み、tyushiはNOT APPLICABLE。**register-appのdelete-modalに同型の別FindingがRC作成中に新規発見された(本Fix対象外、未着手)**。詳細はfamily-d-focus-restoration-close-1.md参照** |
