...
| schedule-app | ✅ | ✅ | ✅(Pattern1) | ✅(端点循環) | ✅**Fix済み・Production反映済み(commit 6e9ddb6、main HEAD `64e026b`、WCAG-JIS-FIX-FAMILY-J-1-RELEASE)。print-modal/img-modalの2モーダルで確認(new-modalは初期focus自体が無く別問題、対象外)。独自の防御分岐[`modal.contains(active)`]は本失敗モードには無効だったが、境界修正で解消** | ✅ **参照実装** | 🟡**FIXED IN RC / USER REVIEW PENDING(WCAG-JIS-FIX-FAMILY-D-BATCH-1、branch: fix/family-d-schedule-modal-restoration、Production未反映)**: print-modalは✅Production反映済み(commit `d729585`)。new-modal/img-modalはWCAG-JIS-FAMILY-D-CROSS-APP-AUDIT-1でCONFIRMED FAIL(new-modal LEVEL-A、img-modalはLEVEL-B)と確定した後、本BatchでRC実装。`newModalOpener`(cancel/confirm/Escape/背景クリック全経路)、img-modalは実trigger(`.item-thumb`、tabindexなし)がfocus不能なため`#tab-editor`への固定fallback。Browser ValidationでLEVEL-A取得済み(全close経路でBODY退行なし)、User Browser Review待ち | — (未検証、new-modal/img-modalは対象外) | 🟡(系統C、inertなし) | 背景inertなしでも端点循環が機能する例。詳細はfamily-d-cross-app-audit-1.md参照 |
...
| Focus Restoration | **11**(matching-app・okane-app・nazorin-print・gaze-keyboard[settingsModal]・hiragana-learn・katakana-app・shiritori2・bosai-app・cup_game[helpModal]・ongaku-app[modal-help]・ongaku-app[pin/export/share]) | **1**(schedule-app、print-modalはPASS済みだがnew-modal/img-modalがWCAG-JIS-FIX-FAMILY-D-BATCH-1でRC実装中・User Review待ちのためアプリ単位ではPARTIAL) | 1(register-app、TIER1-F5) | **7**(time-timer・mogura-tataki・scratch-app・janken-app・tokei-app・tyushi・gaze-keyboard[profileModal/hrModal]) |
