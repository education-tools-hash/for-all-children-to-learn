...
| TIER2-F2 | FAMILY-B | P2 | 3アプリ / **4 Finding(modal単位、2026-09-08 WCAG-JIS-FAMILY-B-LEDGER-NORMALIZE-1で個別Finding ID化。内訳: cup_game[settingsOverlay]=TIER2-F2-a、cup_game[helpOverlay]=TIER2-F2-b、hiragana-learn[traceSampleViewer]=TIER2-F2-c、katakana-app[traceSampleViewer]=TIER2-F2-d。旧「5」は行単位の暫定値でFinding数と異なる単位のため混同しないこと。正式なSource of Truthは`family-b-focus-trap-finding-ledger.md`)** | hiragana-learn・katakana-app・cup_game(helpModal・settingsOverlay) | **[2026-09-09時点] ALL SUB-FINDINGS FIXED IN RC / USER REVIEW PENDING**(旧表記`Open`から更新。`WCAG-JIS-FIX-FAMILY-B-BATCH-7`で4件全てを現在のProductionコードから再同定し実機確認(LEVEL-A、Root Cause=B1 Tab handler完全欠如で共通)、単一Batchでmogura-tataki[FAMILY-B-BATCH-6-RC4]確立のarchitectureを適用してFix実装。Forward/Reverse各15回・immediate Shift+Tab・outside-focus-guard・A11yパネルcoexistence全てPASS。Production Release完了までは正式Closeとしない。詳細はfamily-b-focus-trap-finding-ledger.md§29参照) | - | あり | あり(同上) |
