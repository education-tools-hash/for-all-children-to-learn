...
|---|---|---|---|---|---|---|---|---|---|---|
| register-app | ✅ | ✅ | 🟡(Pattern3、最初のcontrol) | ✅(端点循環) | ✅(Pattern3のためinitial focus=firstと一致、構造的に非該当) | ❌(TIER1-F2) | ✅ | ✅**TECHNICALLY RESOLVED / PRODUCTION REFLECTED(WCAG-JIS-FIX-FAMILY-D-REGISTER-1-RELEASE、commit `b71cbfb`、Production Validation PASS)**: product-modal(TIER1-F5)。Root Cause = 保存(Save)経路のみ`closeModal()`直前に`renderProducts()`が商品グリッド全体を再生成し、`pmOpenerEl`が指す旧DOMノードがdisconnectedになりBODY退行していた(cancel/Escapeは元々正常だった)。`pmOpenerProductId`を追加保持し、disconnected時は再描画後の同一商品の`.product-card`(新規追加時は`.add-product-card`)へfallback。Production ValidationでLEVEL-A実機確認済み。**別Finding(未着手)**: delete-modalも同型のBODY退行あり(cancel経路、実機LEVEL-A確認) | 🟡(系統B、モーダル自身のみ) | 8モーダル共通ヘルパー、Fix影響範囲大。詳細はfamily-d-focus-restoration-close-1.md参照 |
...
| FAMILY-D | Focus Restoration | **0(technical remediation対象、TIER1-F5含む全て解消済み)。[2026-09-08追記] register-app product-modal(TIER1-F5)はWCAG-JIS-FIX-FAMILY-D-REGISTER-1-RELEASE(commit `b71cbfb`)でProduction解消済み。matching-app・okane-app・schedule-app・scratch-app・cup_game[settingsOverlay]も全てProduction解消済み、tyushiはNOT APPLICABLE。**register-appのdelete-modalに同型の別Finding未着手のまま残存**(TIER1-F5には含まれない新規発見、記録のみ)。詳細はfamily-d-focus-restoration-close-1.md参照** |
