...
(function (root) {
  ...
  // Phase T3-C candidate (Candidate C): shape cost + centroid-relationship
  // cost, normalized by the character's own bbox diagonal so it is
  // dimensionless and comparable in scale to shapeCost (which operates in
  // intrinsic [0,1]-normalized space). Generalizes to any character pair
  // of strokes that are shape-ambiguous once intrinsically normalized but
  // sit in clearly different absolute locations (root cause identified for
  // U's stroke#0/#1 in Phase T3-C Section 3 — NOT character-specific: this
  // term is computed identically for all 46 characters).
  const ASSIGNMENT_POSITION_WEIGHT = 0.15;
  function assignmentCost(userFeat, refFeat, charDiag) {
    const shape = shapeCost(userFeat, refFeat);
    const posDist = Math.hypot(userFeat.centroid.x - refFeat.centroid.x, userFeat.centroid.y - refFeat.centroid.y);
    const posNorm = charDiag > 1e-6 ? posDist / charDiag : 0;
    return shape + ASSIGNMENT_POSITION_WEIGHT * posNorm;
  }

  // Returns { assignment: [refIndexForUser0, refIndexForUser1, ...], totalCost, costMatrix }
  function matchStrokes(userFeatures, refFeatures, charDiag) {
    const n = userFeatures.length;
    const diag = charDiag || 1;
    const costMatrix = userFeatures.map((uf) => refFeatures.map((rf) => assignmentCost(uf, rf, diag)));
  ...
  function evaluateCharacter(userStrokes, referenceStrokeDefs, opts) {
    ...
    const charDiagForAssignment = Math.hypot(charBBox.width, charBBox.height);
    const { assignment, costMatrix } = matchStrokes(userFeatures, refFeatures, charDiagForAssignment);
