...
|---|---|---|---|
| P0 Critical | 0 | 0 | **0** |
| P1 High | 0 | 2(TIER2-F3, F6) | **2** |
| P2 Medium | 5(Finding単位) | 3(TIER2-F1, F2, F5) | **8** |
| P3 Low | Contrastの一部(個別評価待ち) | 1(TIER2-F4、15件) | Contrast系はいずれもTier分類基準で個別評価 |

**AUDIT PILOT BLOCKING FINDING相当(P0操作不能)は29アプリ通じて0件。**

Tier2で初めてP1 Highが計上された点が最大の変化(TIER2-F3: 初期focus欠如、TIER2-F6: スイッチスキャン設定トグルの無名化)。いずれもAudit plan v1.1の標準severity基準に従った分類であり、Tier1に同種の問題がなかったことによる偶然の差ではなく、Tier2側で実際に検出されたことによる増加。
...
2. katachi-awase-app(Tier2、TIER2-F6のスイッチスキャン設定無名化の実体験確認)
...
1. **即時横展開可能(正解確立済み)**: TIER2-F6(1行修正)→TIER1-F1/TIER2-F3/TIER1-F2/TIER2-F1(いずれも確立済みパターンの横展開)
...
- 29アプリを通じてP0は0件だが、Tier2で初めてP1が2件検出された。**「Tier3は残り6アプリで規模が小さいから軽微」という前提は置かず、Tier1/Tier2と同水準の精査を行うことを推奨**
