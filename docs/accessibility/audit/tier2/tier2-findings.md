...
> **[2026-09-07更新]** WCAG-JIS-FIX-MODAL-ONGAKU-1でongaku-app(modal-pin/export/share)分を`donomana-modal-accessibility-contract-v1_0.md`準拠で修正し、WCAG-JIS-FIX-MODAL-ONGAKU-1-RELEASEでcommit `edd6299`をmainへ統合済み(main HEAD `373b3d3`、**Production反映済み**)。**ongaku-app分は Production Evidence付きで TECHNICALLY RESOLVED、他3アプリ(hiragana-learn/katakana-app/cup_game)分は引き続きOPEN。Finding Family全体としてはPARTIALLY RESOLVED。**
...
> **[2026-09-07更新]** WCAG-JIS-FIX-P1-B-RELEASEでcup_game分を修正・Production反映済み(commit `a53f307`)。**WCAG-JIS-FIX-MODAL-ONGAKU-1-RELEASEでongaku-app分(modal-pin/export/share)も修正・commit `edd6299`をmainへ統合済み(main HEAD `373b3d3`、Production反映済み)。Finding対象の全アプリがProduction Evidence付きでTECHNICALLY RESOLVEDとなったが、NVDA/VoiceOver Manual Validation Pendingのため完全Closedとはしない。**
...
| Apps | cup_game(helpModal) — ✅ **TECHNICALLY RESOLVED**(commit `a53f307`、Production反映済み)。ongaku-app(modal-pin・modal-export・modal-share) — ✅ **TECHNICALLY RESOLVED**(commit `edd6299`、Production反映済み、main HEAD `373b3d3`) |
| Severity | **P1 High**(維持。両アプリともProduction ReleaseによりTechnically Resolvedが確定したが、NVDA/VoiceOver Manual Validation Pendingが残るため、正式Closeへの降格はManual Review完了後の判断とする) |
...
| 修正内容(ongaku-app) | `donomana-modal-accessibility-contract-v1_0.md`準拠でmodal-pin/modal-export/modal-shareへrole="dialog"・aria-modal・aria-labelledby・initial focus(title、tabindex="-1")・Focus Trap(端点循環型)・A11yパネル例外・Escape・Focus Restoration・背景inertを一括実装。commit `edd6299`、branch `fix/ongaku-modal-accessibility-contract`(WCAG-JIS-FIX-MODAL-ONGAKU-1)。Reverse Tab境界修正(FAMILY-J対策、commit `891fd90`)込みでWCAG-JIS-FIX-MODAL-ONGAKU-1-RELEASEにてmain統合済み(main HEAD `373b3d3`)。TIER2-F2のongaku-app分(Focus Trap欠如)も同時に解消。Regression Contract 14 Gate全PASSを確認、**Production反映済み** |
| Evidence | cup_game.html(修正後、Production反映済み)、ongaku-app.html(修正後、Production反映済み)、`tools/accessibility-audit/tier2/`各種結果 |
...
> **[2026-09-07更新]** WCAG-JIS-FIX-P1-A-REVIEWでTIER2-F6をP1→P2へ再分類(実機Evidenceに基づく訂正、上記TIER2-F6参照)。WCAG-JIS-FIX-P1-B-RELEASEでTIER2-F3のcup_game分をTECHNICALLY RESOLVEDとしてProduction反映(commit `a53f307`)。**WCAG-JIS-FIX-MODAL-ONGAKU-1-RELEASEでTIER2-F3のongaku-app分、およびTIER2-F2のongaku-app分もTECHNICALLY RESOLVEDとしてmainへ統合済み(commit `edd6299`、main HEAD `373b3d3`、Production反映済み)。** TIER2-F3は対象アプリ全てProduction Evidence付きでTechnically Resolvedとなったが、NVDA/VoiceOver Manual Validation Pendingのため件数上はP1を維持。TIER2-F2は他3アプリ(hiragana-learn/katakana-app/cup_game)分が現存するためP2を維持。下表は更新後の値。

| Severity | 件数 |
|---|---|
| P0 Critical | 0 |
| P1 High | 1(TIER2-F3、全対象アプリProduction Evidence付きでTechnically Resolved。NVDA/VoiceOver Manual Validation Pendingのため件数維持) |
