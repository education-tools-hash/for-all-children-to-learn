...
(function(){
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    if (!btn || !panel) return;
    // WCAG-JIS-A11Y-PANEL-STRICT-CONTAINMENT-GLOBAL-1A: 共通A11yパネル自体のkeyboard
    // behavior用helper(donomana-a11y-panel-keyboard-contract-v1_0.md準拠)。
    // window経由で公開し、各アプリ固有スクリプト(Pilot対象)から呼び出せるようにする。
    // 定義自体は全35アプリへ共通反映されるが、実際にTab/Shift+Tabのcontainmentを
    // 発火させる(trapA11yPanelFocusを呼び出す)かどうかはPilotアプリ側の個別実装に
    // 委ねる(helperの存在とfeature activationを分離、Non-Pilotアプリの挙動は不変)。
    function donomanaA11yElFocusable(el){
    ...
    function getA11yPanelFocusables(panelEl){
      return Array.prototype.filter.call(
        panelEl.querySelectorAll('button,input,select,textarea,a[href],[tabindex]'),
        donomanaA11yElFocusable
      );
    }
    function trapA11yPanelFocus(e){
      if (e.key !== 'Tab' || panel.style.display !== 'block') return false;
      var items = getA11yPanelFocusables(panel);
      if (!items.length) return false;
      var first = items[0], last = items[items.length - 1];
      var active = document.activeElement;
      var inPanel = items.indexOf(active) !== -1;
      if (e.shiftKey) {
    ...
    function restoreA11yPanelFocus(){
      if (donomanaA11yElFocusable(btn)) btn.focus();
    }
    window.getA11yPanelFocusables = getA11yPanelFocusables;
    window.trapA11yPanelFocus = trapA11yPanelFocus;
    window.restoreA11yPanelFocus = restoreA11yPanelFocus;
    ...
    document.addEventListener('keydown', function(e){
      // WCAG-JIS-A11Y-PANEL-STRICT-CONTAINMENT-GLOBAL-1A: 旧btn.click()による
      // 合成クリックはbtn自体へfocusを移動させないため、Panel内部(opener以外)に
      // focusがある状態でEscapeを押すとdocument.activeElementがBODYへ落ちる
      // (mogura-tatakiで実機確認済み、共通層コードが原因の35アプリ共通の構造的
      // リスク)。close処理を直接実行したうえでrestoreA11yPanelFocus()により
      // openerへ明示的にfocusを戻す。
...
function buildLearningRecordFoundationJSHTML() {
  return [
    ...
    '// Phase T7-H1: this function\'s own documented contract (Standard §9 —',
    '// "存在しない/壊れている場合は[]") already promised an array-or-[] return, but the',
    '// implementation only guarded a missing key and a JSON.parse failure — a storage',
    '// value that is valid JSON but NOT an array (e.g. {}, "hello", 123, true, null, an',
    '// object) parsed successfully and was returned as-is, silently breaking that',
    '// contract. donomanaRecordAddLog() (below) trusts this return value and calls',
    '// .push() on it directly with no guard of its own, so that gap surfaced there as',
    '// "log.push is not a function" (or a TypeError reading a property of null) instead',
    '// of here. Fixing the single point where raw storage is parsed — rather than adding',
    '// a second Array.isArray check in every caller — makes every caller (Add and any',
    '// direct Read caller across the 15 Foundation apps) safe at once, matching the',
    '// per-app defensive `Array.isArray(log) ? log : []` wrapper a few apps (e.g.',
    '// katachi-awase-app\'s own readLog()) already had to add locally for the same reason.',
    ...
    '  } catch (e) { return []; }',
    '}',
    '// Phase LEARNING-RECORD-STORAGE-SAVE-SAFETY-1: setItem失敗(QuotaExceededError等)を',
    '// silentに握りつぶさない。呼び出し元はdonomanaRecordWriteLog()の戻り値、または',
    '// donomanaRecordLastSaveResult()で直近の書き込み結果を判定できる。失敗時は',
    '// donomanaRecordNotifySaveFailure()が自動的に利用者へ通知する(視覚+スクリーンリーダー)。',
    '// 既存key/schema/呼び出し方は一切変更しない(戻り値を見ない既存呼び出し元は今まで通り',
    '// 動作する。Root Cause: docs/records/learning-record-storage-capacity-audit-v1_0.md §4)。',
    ...
    '  } catch (e) {}',
    '  if (typeof window.donomanaAnnounce === \'function\') {',
    '    try { window.donomanaAnnounce(msg, { priority: \'assertive\' }); } catch (e2) {}',
    '  }',
    '}',
    ...
    '  } catch (e) {',
    '    __donomanaLastSaveResult = { ok: false, reason: donomanaRecordClassifySaveError(e) };',
    ...
    '  } catch (e) { return []; }',
    '}',
    '// Phase LEARNING-RECORD-STORAGE-SAVE-SAFETY-1: setItem失敗をsilentに握りつぶさない。',
    '// 戻り値はdonomanaRecordLastSaveResult()と同じ{ok,reason}形式(呼び出し元が既存のまま',
    '// 戻り値を使わなくても壊れない)。失敗時はdonomanaRecordNotifySaveFailure()が自動通知する。',
...
const MANUAL_CHANGELOG = [
  { date: "2026-09-22", type: "update", text: "「カタカナ まなぼう！」と「マッチング」のスイッチ操作を改善しました。", details: [
    "スイッチスキャン中にスペースキーやEnterキーを1回押すと、選んでいる項目だけが1回反応するようにしました。",
    "スイッチを長く押したときに、同じ項目が繰り返し反応しないようにしました。",
    "「マッチング」のアクセシビリティ設定画面を、スイッチスキャンで選べるようにしました。",
    "「マッチング」の詳しい設定画面で、現在選んでいる項目を黄色い枠で確認できるようにしました。"
  ], verbatim: true },
  { date: "2026-09-19", type: "update", text: "学習の記録で、画像や軌跡を含む完全バックアップを書き出せるようにしました。", details: [
    "「なぞり書き」「ひらがな」「カタカナ」「さわってひろがる」の記録詳細から、完全バックアップ（JSON形式）を保存できます。",
    "なぞった画像や、なぞりの線の軌跡など、CSVには含まれない情報もそのまま保存されます。",
    "これまでのCSV保存は、今までどおりお使いいただけます。",
    "バックアップの書き出しでは、端末内の記録は変更されません。",
    "ファイル名に、お子さまのお名前などは含まれません。"
  ], verbatim: true },
  // 同日3件目(下の2件に続く)。CHANGELOG-DATE-SAME-DAY-ENTRY-1で対応済みのため、
  // 同日に何件あっても個別entryのまま正常表示される。
  { date: "2026-09-18", type: "update", text: "学習の記録の保存を、より安心してお使いいただけるようにしました。", details: [
    "端末の空き容量が少ない場合など、記録の保存がうまくいかなかったときに、画面にわかりやすいお知らせが表示されるようになりました。",
    "保存がうまくいかなかった場合でも、それまでに保存されていた記録が消えることはありません。",
    "お知らせは画面表示だけでなく、読み上げ機能でも確認できます。",
    "一時的な問題が解消すれば、そのまま記録を続けてお使いいただけます。"
  ], verbatim: true },
  // 同日2件目(ひらがな・カタカナに続く)。CHANGELOG-DATE-SAME-DAY-ENTRY-1で
  // 対応済みのため、同日に何件あっても個別entryのまま正常表示される。
  { date: "2026-09-18", type: "update", text: "「おかね」「とけい」「しりとり」の学習記録を見やすくしました。", details: [
    "共通の「学習の記録」から、取り組んだ内容の主な詳しい情報を確認できるようになりました。",
    "「とけい」は、取り組んだ問題の形式（よむもんだい・みつけるもんだい・まぜまぜ）を正しく表示します。",
    "「しりとり」は、取り組んだ問題数のモードを正しく表示します。",
    "共通の「学習の記録」から、詳しい記録をCSV形式で保存できます。"
  ], verbatim: true },
  { date: "2026-09-18", type: "update", text: "ひらがな・カタカナの学習記録で、なぞった線を確認できるようになりました。", details: [
    "保存されたなぞりの線を「学習の記録」から確認できます。",
    "1画だけでなく、複数の画もそれぞれ確認できます。",
    "対象文字やストローク数などの情報も確認できます。",
    "アプリ内と共通「学習の記録」で同じ記録を確認できます。",
    "詳しい記録をCSV形式で保存できます。"
  ], verbatim: true },
  ...
  // 同日3件目(SST・directions-appに続く)。CHANGELOG-DATE-SAME-DAY-ENTRY-1で
  // 対応済みのため、同日に何件あっても個別entryのまま正常表示される。
  { date: "2026-09-17", type: "update", text: "「おおきい？ちいさい？くらべよう」「かたちをあわせよう」の学習記録を詳しく確認できるようになりました。", details: [
    "「おおきい？ちいさい？くらべよう」の詳しい学習記録を確認できます。",
    "「かたちをあわせよう」の詳しい学習記録を確認できます。",
    "問題内容や選んだ内容、正解・間違い、反応時間などを確認できます。",
    "「かたちをあわせよう」は、形・大きさ・パズルごとの記録を確認できます。",
    "アプリ内の記録と共通の「学習の記録」で、同じ内容を確認できます。",
    "詳しい記録をCSV形式で保存できます。"
  ], verbatim: true },
  // 同日4件目(SST・directions-app・kurabeyou/katachiに続く)。同じくCHANGELOG-
  // DATE-SAME-DAY-ENTRY-1で対応済みのため、同日に何件あっても個別entryのまま
  // 正常表示される。
  { date: "2026-09-17", type: "update", text: "「なぞり書き練習ツール」の学習記録で、なぞった画像を確認できるようになりました。", details: [
    "保存されたなぞり書きの画像を、共通の「学習の記録」画面からも確認できるようになりました。",
    "1文字ずつ取り組んだ場合は、文字ごとの画像をまとめて確認できます。",
    "続けて書くモードで取り組んだ場合の画像も確認できます。",
    "画像とあわせて、取り組んだ文字数などの情報も確認できます。",
    "アプリ内の記録と共通の「学習の記録」で、同じ内容を確認できます。",
    "詳しい記録をCSV形式で保存できます。"
  ], verbatim: true },
  { date: "2026-09-16", type: "update", text: "「さわってひろがる」の学習記録を詳しく確認できるようになりました。", details: [
    "タップした回数・スワイプした回数など、操作の内容を詳しく記録できるようになりました。",
    "「せんせい用せってい」で「操作の軌跡を記録」をONにすると、画面のどこを触ったかの軌跡をあとから確認できます（初期設定はOFF、視線を使った反応の位置は記録しません）。",
    "アプリ内の「きろくをみる」だけでなく、共通の「学習の記録」画面からも同じ内容を確認できるようになりました。",
    "きろくの記録・軌跡の記録は、それぞれCSV形式で保存できます。"
  ], verbatim: true },
  { date: "2026-09-13", type: "update", text: "SSTの学習記録機能を強化しました。", details: [
    "SSTで取り組んだ場面や問題文、提示された選択肢、選んだ回答などを、これまでより詳しく記録できるようになりました。",
    "記録画面で学習の様子を振り返れるほか、CSV形式での保存にも対応しています。"
  ], verbatim: true },
  { date: "2026-09-06", type: "update", text: "一部の教材の使いやすさと、『すごろく』のマスの内容を改善しました。", details: [
    "一部の教材で、キーボード操作や設定まわりの使いやすさを改善しました。",
    "一部の教材で、スイッチ操作や全画面表示ボタンの使いやすさを改善しました。",
    "『すごろく』のマスの内容や表示を改善しました。"
  ], verbatim: true },
  ...
  { date: "2026-08-31", type: "update", text: "学習記録機能の改善と、新しいパズル遊びの追加をしました。", details: [
    "「もぐらたたき」で、ホーム画面から学習記録を確認できるようにしました。",
    "「とけい」で、クイズの学習記録を保存して、あとから確認できるようにしました。",
    "「とけい」の学習記録のCSV書き出しを、見やすい形式に改善しました。",
    "「かたちをあわせよう」に、色や形を組み合わせて絵を完成させる新しい遊び方「かたちでつくろう」を追加しました。20種類のパズルを、タッチ・視線入力・スイッチなど、いろいろな操作方法で楽しめます。",
    "学習記録の保存処理を改善しました。",
    "「視線キーボード」のWord保存を改善しました。Wordファイルが重複して保存される問題を修正し、DOCX形式で保存できるようにしました。",
    "「なぞり書き練習ツール」の学習記録機能を改善しました。「ぼうさいたんけんたい」では、記録を端末に保存して、あとから振り返れるようにしました。",
    "「マッチング」と「しりとりあそび」に学習記録機能を追加しました。あそび終わるたびに記録が端末に保存され、あとから振り返ったり、CSVで書き出したりできます。",
  ], verbatim: true },
  { date: "2026-08-30", type: "update", text: "教材の使いやすさと学習記録、視線キーボードの機能を改善しました。", details: [
    ...
    "『なぞり書き練習ツール』の活動記録CSVを、日付・時刻・練習した文字・進み具合・活動時間が分かりやすい形式に改善しました。",
    "『とけい』『ぼうさいたんけんたい』『じゃんけん まなぼう！』など11の教材に「つかいかた」を追加・改善し、はじめて使う先生や支援者の方にも、教材の目的や操作方法がすぐに分かるようにしました。",
  ], verbatim: true },
