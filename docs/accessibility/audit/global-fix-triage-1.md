...
| TIER1-F3 | FAMILY-B | P2 | 6[2026-09-08、register-app(delete-modal)を新規追加] | mogura-tataki・scratch-app(txtEdOv・cov)・nazorin-print(help/batch/lib)・tyushi(help-overlay。settings-panelは非modal確定によりNOT APPLICABLE、対象外)・gaze-keyboard(profileModal/hrModal)・**register-app(delete-modal)** | Open | - | あり(Modal Accessibility Contract v1.1で正式要件確定済み) | あり(register-app product-modal等の端点循環実装を参照実装として適用可能。詳細はfamily-b-cross-app-audit-1.md参照) |
| TIER2-F2 | FAMILY-B | P2 | 5[2026-09-08、cup_game(settingsOverlay)を新規追加、ongaku-app分を訂正] | hiragana-learn・katakana-app・cup_game(helpModal・**settingsOverlay**) | Open | - | あり | あり(同上) |
