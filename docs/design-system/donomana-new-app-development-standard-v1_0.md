# どのまな 新規アプリ開発標準仕様書 v1.7

- 版: v1.7
- 発行: 2026-08-12（v1.0）／改訂: 2026-08-13（v1.1）／改訂: 2026-08-15（v1.2）／改訂: 2026-08-19（v1.3）／改訂: 2026-08-19（v1.4）／改訂: 2026-08-19（v1.5）／改訂: 2026-08-19（v1.6）／改訂: 2026-09-06（v1.7）
...
| v1.5 | 2026-08-19 | Phase M11.3-B（Multi-Input Pilot 3教材へのGaze Accessibility Standard実装完了）・M11.3-C（Final Consistency Audit・junban-miyou-app.html設定ボタン不具合修正）を反映。1章の文書地図のGaze Accessibility Standard行を「v1.0（改訂1）」から「v1.0（改訂2）」・状態を「Phase M11.3-A/B/CでMulti-Input Pilot 3教材へ実装完了」へ更新。本文（13章等）の変更なし |
| v1.6 | 2026-08-19 | Phase M11.4-B。1章の文書地図のGaze Accessibility Standard行のバージョン表記が、Gaze Accessibility Standard側のその後の改訂（Phase M11.4-A「改訂3」・M11.4-B「改訂4」、Gaze Shared Foundation関連）に追随できておらず「v1.0（改訂2）」のまま停滞していたのを是正し、「v1.0（改訂4）」へ更新（参照ポインタのみの単純修正、本文（13章等）の変更なし） |
| v1.7 | 2026-09-06 | Changelog Simplification Policy正式化。2026-09-06に9件の技術寄りdetails（Contact Form Hotfix・Sugoroku Hotfix等、複数教材・複数改善カテゴリにまたがる細かな修正）が単一entryへ積み上がり、44章の既存ルール（44.3 1 app × 1 semantic category = 1 item、44.6/44.7 同一アプリ改善統合）だけではカバーしていなかった「横断的な小修正の統合」「同一日の件数目安」「個別記載すべき例外」「Changelog／commit／Phase report／audit docsの役割分担」を44.11〜44.15として新設。既存章（44.1〜44.10）の内容・`generateChangelog()`のarchitectureは変更していない |
...
| `docs/design-system/donomana-gaze-accessibility-standard-v1_0.md` | v1.0（改訂4） | **確定（Phase M11.3-A/B/CでMulti-Input Pilot 3教材へ実装完了。M11.4-A/BでGaze Shared Foundation化完了）** | 視線入力（Gaze/Dwell）固有の設定・UI・lifecycle・Multi-Input統合要件。本書13章の詳細補足文書 |
...
### 44.10 semantic merge の実装（Phase26-Eで発見・Phase26-Fで解消）

Phase26-E時点では、`generateChangelog()`の集約処理が、`MANUAL_CHANGELOG`の`details[]`内でアプリ名を検出した行を、公開/改善のいずれであったかに応じて`「〇〇〇」を公開しました`／`「〇〇〇」を公開し、改善しました`／`「〇〇〇」を改善しました`／`「〇〇〇」を見やすく改善しました`という4種の定型文へ**置き換えており**、`details[]`に書かれた具体的な△△△（見やすさ・操作性等）の語を保持しない既知のギャップがあった。

Phase26-Fでこれを解消した。現在の`generateChangelog()`は、そのアプリを名指しした`details[]`行がその日1本だけの場合、その原文をそのまま表示へ使う（**Structure may be normalized; semantics must be preserved** — 1 app = 1 itemへの構造的集約は行うが、著者が書いた具体的な文言は破棄しない）。同日に新規公開と改善が両方存在するアプリは、`「〇〇〇」を公開し、改善しました`のような1行への一般化はせず、`「〇〇〇」を公開しました`と改善内容の行を意味の異なる2項目として保持する（44.3のsemantic category ruleに対応）。

ただし、同じアプリを指す`details[]`行が同日に複数本あり、かつまだ1文へ統合されていない場合（44.6/44.7が想定する「執筆時に人が統合する」運用がまだ行われていない過去データ等）は、無理な自動合成は行わず、従来通りの一般化文（`「〇〇〇」を改善しました`等）へフォールバックする。この場合は44.6/44.7に従い、`MANUAL_CHANGELOG`側で1文へ統合することが引き続き推奨される。

### 44.11 横断的な小修正の統合（Cross-App Minor Fix Grouping）

44.3〜44.7は同一アプリ内の統合を扱う。一方、複数の異なるアプリへ同種の小さな操作性・アクセシビリティ改善を横断的に行った場合（例: 複数教材のキーボード操作改善、複数教材のフォーカス制御修正）、アプリ名を列挙せず、改善カテゴリ単位で1〜2項目へまとめる。

避ける例（教材ごとに同内容を並べる）:

> 「〇〇〇」のキーボード操作を改善しました
> 「△△△」のキーボード操作を改善しました
> 「□□□」のせってい画面のフォーカス制御を改善しました

推奨する例（カテゴリ単位へ統合し、対象を「一部の教材で」と一般化する）:

> 一部の教材で、キーボード操作や設定まわりの使いやすさを改善しました
> 一部の教材で、スイッチ操作や全画面表示ボタンの使いやすさを改善しました

対象アプリ名を個別に列挙する必要はない。利用者にとって重要なのは「どのまな全体で継続的に使いやすさが改善されている」ことであり、対象アプリの網羅列挙ではない。

### 44.12 件数目安（Volume Guideline）

同一日付のentryは、`details[]`の項目数として原則**3〜5項目程度**を目安とする。

細かな修正が実際には10件以上発生していても、44.3・44.6・44.7・44.11に従って意味の近いものを統合し、この範囲へ収める。件数を減らすこと自体が目的ではなく、統合の結果として自然にこの範囲へ収まることを目指す。

例外: 新しい教材の複数同時公開、複数の重要な新機能追加など、個別に知らせる教育的・利用者保護上の必要性がある場合は、3〜5項目を超えてよい。件数上限を機械的に適用して重要な告知を握りつぶさない。

### 44.13 個別記載すべきもの（Items Not to Over-Merge）

以下は44.6・44.7・44.11の統合対象とせず、必要に応じて個別項目として残してよい。

- 新しい教材の公開
- 大きな新機能の追加
- 利用者の操作方法・利用の仕方が変わる重要な仕様変更
- データ保存・互換性に関わる重要な変更（例: 保存形式変更、既存データの引き継ぎに関する注意）
- オフライン対応など利用可能範囲の大きな変更
- 重大な不具合の修正（利用者が実際に被害・データ損失等を受けていた可能性があるもの）
- 利用者へ注意喚起が必要な変更

これらは「操作性を改善しました」のような一般化した表現へ丸めず、何が変わったか・利用者が何をすべきかが伝わる具体性を保つ。

### 44.14 Changelogと他の記録媒体の役割分担（Role Separation）

どのまなの変更に関する記録は、対象読者に応じて以下の4層に分離する。1つの層の内容を別の層で代替しない。

| 層 | 対象読者 | 記述粒度 |
|---|---|---|
| User-facing Changelog（`index.html`更新履歴） | 利用者（保護者・支援者・教員等） | 簡潔・利用者視点。「〇〇を改善しました」程度（44.5・44.8） |
| Git commit | 開発者 | 実装単位。何のファイルをどう変更したか |
| Phase final report | 開発者・レビュアー | 検証結果・Root Cause・変更詳細・Scope Guard遵守確認 |
| Audit docs（設計仕様書・監査文書） | 開発者・将来の実装者 | 技術的負債・アーキテクチャ上の判断・既知のギャップの記録 |

44.5で列挙した技術用語（DOM／CSS／JavaScript／`localStorage`／API／Git／commit／Phase番号／internal function名等）は、Changelog層ではなくGit commit・Phase final report・Audit docs層へ記録する。

### 44.15 新規entry追加時の運用手順（Operational Procedure）

`MANUAL_CHANGELOG`へ新しい変更を追記する際は、行を増やす前に以下を確認する。

1. 同日の既存entry（同じ`date`の`details[]`）を確認する
2. 追記しようとしている内容が、既存の同一アプリ・同一改善カテゴリの項目へ意味的に統合できないか確認する（44.7）
3. 統合できる場合は新規行を追加せず、既存行を意味的に拡張する
4. 統合できず新規行が必要な場合も、44.5の推奨語彙・利用者向け表現へ言い換える
5. 44.13の例外に該当しない限り、技術用語・実装詳細・内部関数名・Phase番号を含めない
...
[ ] Console Error 0
[ ] pageerror 0
[ ] Common Chrome（Home / Lock / Fullscreen / A11y等）はgenerate.js注入のみを使用し、独自の重複実装がないか
...
**Case H — 更新履歴の自動集約が具体的な改善内容を一般化 → semantic preservationへ修正**
Phase26-Eで本書自体を改訂する過程で、`generateChangelog()`の集約処理が、同日に新規公開と改善が重なったアプリについて、`MANUAL_CHANGELOG`の具体的な文言を保持せず「「〇〇〇」を公開し、改善しました」という一般化された定型文へ置き換えることを発見した。Phase26-Eでは複数アプリへ影響する共通仕様変更に該当するとしてSTOPし、既知のギャップとして記録するに留めたが、Phase26-Eで確定したChangelog Policyそのものへ実装を整合させる作業として、Phase26-Fで正式に修正した。そのアプリを指す行が同日1本だけの場合は原文を保持し、公開と改善は別項目として保持するよう変更した（44.10章）。

**Case I — 新規アプリが独自Home Navigationを実装したまま公開 → Pre-Production Checklistへ明示チェック追加**
Multi-Input Program 3本目「じゅんばんにみよう」（Phase M10-H登録、2026-08-14公開）は、8章がREQUIREDと定める共通Homeボタン（`donomanaHomeBtn`、generate.js注入）ではなく、独自の「←もどる」リンクを実装したまま公開されていた。M10-H登録時、`generate.js`の`HOME_BTN_SKIP_APPS`（本来は二重挿入回避のための技術的escape hatchであり、正式な例外許可リストではない）へ追加する形で「二重化回避」という技術的対処のみが行われ、8章への適合性は検証されなかった。ユーザーからの指摘（Phase M10-J）を受けて調査した結果、8章の本文記載自体は当時から明確にREQUIREDだったが、56章Pre-Production Checklistには対応する明示チェック項目が存在せず、7章の設定入口二重化（Case D）と異なり、Home Navigationの独自重複実装を検出する仕組みが運用チェックリスト上に欠けていたことが根本原因と判明した。「本文にREQUIREDと書いてあること」と「公開直前チェックリストで実際に確認されること」は別の保証であり、後者が欠けていると本文の強度に関わらず見落とされうる。56章へ明示チェック項目を追加して是正した。
