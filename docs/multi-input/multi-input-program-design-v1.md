# Multi-Input教材群 共通設計 v1.0（Phase M0）
...
以下はM0時点で断定せず、本書では「推奨候補」の形で提示するに留めた。M1着手前にユーザー判断を仰ぐ。

1. **dwell時間**: 固定900ms据え置きか、600/900/1200ms等の可変対応にするか（8.1章）。
2. **初期Levelでmistakeを持つか**: 「みつけてタッチ」Level1（distractorなし）は本書の推奨通りmistake概念なしでよいか（5.6章）。
3. **sensory feedback量**: しずか/ふつう/にぎやか等の設定を持たせるか、それとも固定feedbackで開始するか（9章）。
4. **scan方式**: auto scan既定か、manual scanも初期から用意するか（8.2章）。
5. **input mode UI**: 自動併用（推奨A案）のままでよいか、明示的なMode選択を先に用意するか（8.5章）。
6. **records項目**: 11.2章の候補のうちどこまでを実装対象とするか（全項目実装は過剰な可能性がある）。
7. **development order**: 3教材の開発順序（みるとひろがる→みつけてタッチ→じゅんばんにみよう）でよいか（16.1章）。
