...
|---|---|---|---|---|---|---|---|---|---|---|
| register-app | ✅ | ✅ | 🟡(Pattern3、最初のcontrol) | ✅(端点循環、**product-modalのみ**。delete-modalはFocus Trap自体が実装されておらずShift+TabでモーダルOK外(背景`record-open-btn`等)へ抜けることを実機確認、既存の別Finding・FAMILY-B領域・今回Fix対象外) | ✅(Pattern3のためinitial focus=firstと一致、構造的に非該当) | ❌(TIER1-F2) | ✅ | ✅product-modal(TIER1-F5)はTECHNICALLY RESOLVED / PRODUCTION REFLECTED(commit `b71cbfb`)。🟡**delete-modalはFIXED IN RC / USER REVIEW PENDING(WCAG-JIS-FIX-FAMILY-D-REGISTER-DELETE-1、branch: fix/family-d-register-delete-restoration、Production未反映)**: 全close経路(cancel/Escape/confirm削除)でBODY退行を実機確認(LEVEL-A)。`dmDeletingId`+`dmDeletingIndex`を保持し、cancel/Escapeは同一`.product-card`へ、confirm(実削除)後は再描画後の同一index位置のカード(繰り上がった次カード、無ければ最後のカード、グリッドが空なら`.add-product-card`)へlogical replacement。Browser ValidationでLEVEL-A確認済み(境界ケース: 最後のカード削除・全削除も確認) | 🟡(系統B、モーダル自身のみ) | 8モーダル共通ヘルパー、Fix影響範囲大。詳細はfamily-d-focus-restoration-close-1.md参照 |
...
| FAMILY-D | Focus Restoration | **Production残存0件(register-app product-modal[TIER1-F5]含む既存登録分は全て解消済み)、RC対応中1件(register-app delete-modal、新規発見Finding、Production未反映)。[2026-09-08追記] delete-modalはWCAG-JIS-FIX-FAMILY-D-REGISTER-DELETE-1でRC実装済み、User Review待ち。Production反映まではFAMILY-D residual = 1として扱う。詳細はfamily-d-focus-restoration-close-1.md参照** |
