...
(function (root) {
  ...
  const ENDPOINT_WEIGHT = 0.15;
  function endpointCost(uf, rf, charDiag) {
    const shape = shapeCost(uf, rf);
    const d1 = Math.hypot(uf.start.x-rf.start.x, uf.start.y-rf.start.y) + Math.hypot(uf.end.x-rf.end.x, uf.end.y-rf.end.y);
    const d2 = Math.hypot(uf.start.x-rf.end.x, uf.start.y-rf.end.y) + Math.hypot(uf.end.x-rf.start.x, uf.end.y-rf.start.y);
    const best = Math.min(d1, d2) / 2;
    const norm = charDiag > 1e-6 ? best / charDiag : 0;
    return shape + ENDPOINT_WEIGHT * norm;
  }
  function matchStrokes(userFeatures, refFeatures, charDiag) {
    const n = userFeatures.length;
    const diag = charDiag || 1;
    const costMatrix = userFeatures.map((uf) => refFeatures.map((rf) => endpointCost(uf, rf, diag)));
