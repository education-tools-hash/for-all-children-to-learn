...
| 2 | TIER2-F2-a | cup_game | cup_game.html | `settingsOverlay` | Modal Dialog(role/aria-modal無し、別Finding) | YES(実質modal) | YES | **LEVEL-A**(2026-09-09実機再確認、Forward Tab#1で即座にBODY外へ脱出を確定) | **PASS**(15回連続Tabで脱出せず。pre-fix: Tab#1で即座に脱出) | **PASS**(15回連続Shift+Tabで脱出せず) | **PASS**(`settingsCloseBtn`へ正しくwrap) | **PASS**(`donomanaA11yBtn`から強制focusした状態から復帰確認) | B1 | P2 | **✅ TECHNICALLY RESOLVED / PRODUCTION REFLECTED**(`WCAG-JIS-FIX-FAMILY-B-BATCH-7-RELEASE`, commit `24dd18e`。User Browser Review Approved、Production HEAD上で再検証PASS済み) | Batch 7(Fix実装は今回が初めて。旧記載「Batch 3」の由来は不明、Note欄で確認できず) | A11yパネルProxy経由が唯一の到達経路。Initial Focus欠如(既存FAMILY-C Finding、今回修正せず)。custom toggle(`toggleDwell`等)のVisible Focus問題を新規発見、Separate Finding候補として記録(今回修正せず) |
| 3 | TIER2-F2-b | cup_game | cup_game.html | `helpOverlay`(helpModal) | Modal Dialog(内側`#helpModal`に`role="dialog" aria-modal="true"`developer明示) | YES | YES | **LEVEL-A**(2026-09-09実機再確認、Forward Tab#2でBODY外へ・Reverse Shift+Tab#1で即座に脱出を確定) | **PASS**(15回連続Tabで脱出せず。pre-fix: Tab#2で脱出) | **PASS**(15回連続Shift+Tabで脱出せず。pre-fix: Shift+Tab#1で即座に脱出) | **PASS**(`helpCloseBtn`へ正しくwrap) | **PASS**(強制focusした状態から復帰確認) | B1 | P2 | **✅ TECHNICALLY RESOLVED / PRODUCTION REFLECTED**(`WCAG-JIS-FIX-FAMILY-B-BATCH-7-RELEASE`, commit `24dd18e`) | Batch 7(Fix実装は今回が初めて) | 既存Initial Focus(`helpTitle`)・Focus Restoration(`donomanaHelpBtn`)・Escapeともregressionなしを実機確認。修正実装中に`tabindex="-1"`のInitial Focus anchorをfocusables境界計算から除外する際の設計上の教訓を2点発見(詳細は§29参照) |
...
| 12 | TIER2-F2-c | hiragana-learn | hiragana-learn.html | `traceSampleViewer` | Modal Dialog(`role="dialog" aria-labelledby`、aria-modal無しは別Finding) | YES | YES | **LEVEL-A**(2026-09-09実機再確認によりLEVEL-Bから昇格、Forward Tab#3でBODY外へ・Reverse Shift+Tab#1で即座に脱出を確定) | **PASS**(15回連続Tabで脱出せず。pre-fix: Tab#3で脱出) | **PASS**(15回連続Shift+Tabで脱出せず。pre-fix: Shift+Tab#1で即座に脱出) | **PASS**(`traceSampleCloseBtn`へ正しくwrap) | **PASS**(強制focusした状態から復帰確認) | B1 | P2 | **✅ TECHNICALLY RESOLVED / PRODUCTION REFLECTED**(`WCAG-JIS-FIX-FAMILY-B-BATCH-7-RELEASE`, commit `24dd18e`) | Batch 7(Fix実装は今回が初めて) | katakana-appと共通実装(コード共有、ファイルは別、Fixも個別に同一実装を追加)。既存Initial Focus(`traceSampleTitle`)・Focus Restoration(`traceSampleViewerPrevFocus`)・Escapeともregressionなしを実機確認。`tabindex="-1"`要素の扱いに関する設計上の教訓を2点発見(詳細は§29参照) |
| 13 | TIER2-F2-d | katakana-app | katakana-app.html | `traceSampleViewer` | Modal Dialog(同上) | YES | YES | **LEVEL-A**(2026-09-09実機再確認によりLEVEL-Bから昇格、hiragana-learnと同一パターンで脱出を確定) | **PASS**(15回連続Tabで脱出せず。pre-fix: Tab#3で脱出) | **PASS**(15回連続Shift+Tabで脱出せず。pre-fix: Shift+Tab#1で即座に脱出) | **PASS**(`traceSampleCloseBtn`へ正しくwrap) | **PASS**(強制focusした状態から復帰確認) | B1 | P2 | **✅ TECHNICALLY RESOLVED / PRODUCTION REFLECTED**(`WCAG-JIS-FIX-FAMILY-B-BATCH-7-RELEASE`, commit `24dd18e`) | Batch 7(Fix実装は今回が初めて) | hiragana-learnと共通実装(Fixも個別に同一実装を追加)。既存Initial Focus・Focus Restoration・Escapeともregressionなしを実機確認 |
...
## 30. [2026-09-09追記] WCAG-JIS-FIX-FAMILY-B-BATCH-7-RELEASE完了

RC(§29)についてUser Browser Reviewを実施したところ、**User Approved**を得た。

### Release手順

1. baseline再確認: `origin/main`は引き続き`7b784f5`(RC作成時baselineと一致、drift無し)。意味的競合評価は不要と判断。
2. Release直前のRC最終検証(Batch-7 worktree): TIER2-F2-a〜d全件でForward/Reverse各15回・immediate Shift+Tab・outside-focus-guard・Escape/Focus Restoration regressionを再確認、全てPASS。
3. mainのworktree(`C:/Users/jerry/Documents/GitHub/for-all-children-to-learn`)で`git merge --ff-only fix/family-b-tier2-focus-trap`を実行、`7b784f5`→`24dd18e`へfast-forward(driftが無かったため単純なfast-forwardを採用、rebase/cherry-pickは不要と判断)。
4. Production相当HEAD(mainのworktree)上で全項目を再検証: TIER2-F2-a〜d全件のForward/Reverse各15回・immediate Shift+Tab・outside-focus-guard・A11yパネルcoexistence(Forward/Reverse各15回+close後Trap復帰)・Escape/Focus Restoration regression・Responsive(375×667/390×844/768×1024/1280×900/200%zoom、horizontal overflow無し・close button可視)・Touch、いずれもPASS。Console/page errors 0件。
5. Static Validation: `git diff --check`エラーなし、変更ファイルは意図した6ファイル(`cup_game.html`/`hiragana-learn.html`/`katakana-app.html`/Ledger3点)のみで対象外アプリへの混入なし、ID重複なし、`aria-labelledby`/`aria-controls`参照先破損なし、keydownリスナー数は各ファイルで意図した+1件の増分のみ(cup_game: 4→5、hiragana-learn/katakana-app: 5→6)。
6. §3個別Findingテーブルの`TIER2-F2-a`〜`d`のStatusを`✅ TECHNICALLY RESOLVED / PRODUCTION REFLECTED`へ更新(RC表記を削除)。
7. `git push origin main`実施。
8. CI確認(`git fetch origin`+`git rev-parse origin/main`): push後、`cup_game.html`/`hiragana-learn.html`/`katakana-app.html`は`apps-data.json`のスキーマ変更を伴わないため`generate.js`の出力に差分が生じず、CI自動コミットはスキップ(想定どおりの正常挙動、`git diff --staged --quiet || git commit`仕様に合致)。
9. `git merge-base --is-ancestor`で`61f9a47`・`24dd18e`双方がProduction history(`origin/main`)に含まれることを確認。

### Production反映内容

- `TIER2-F2-a`(cup_game settingsOverlay)・`TIER2-F2-b`(cup_game helpOverlay)・`TIER2-F2-c`(hiragana-learn traceSampleViewer)・`TIER2-F2-d`(katakana-app traceSampleViewer)の4 Finding全てが**✅ TECHNICALLY RESOLVED / PRODUCTION REFLECTED**。

### Production residual count再計算(推測せず、正規化された18件の集計単位で確認)

`WCAG-JIS-FAMILY-B-LEDGER-NORMALIZE-1`で正規化されたFAMILY-B(Focus Trap)の全18 Finding(`TIER1-F3-a`〜`n`の14件 + `TIER2-F2-a`〜`d`の4件)について§3テーブルの各行のStatusを確認したところ、**18件全てが`✅ TECHNICALLY RESOLVED / PRODUCTION REFLECTED`**であることを確認した。

- **FAMILY-B(Focus Trap)Production residual count: 4→0**
- ongaku-app(pin/export/share)は既にFix済みのため元々この18件のカウント対象外(変更なし)。
- tyushi(settings-panel)は非modal確定によりN/A、この18件のカウント対象外(変更なし)。
- ただし、FAMILY-B(Focus Trap)としてはresidual 0だが、**関連するSeparate Finding(FAMILY-Bの直接スコープ外)は複数継続中**であり、それらはFocus Trapとは別カテゴリの問題として残存している(下記「継続するSeparate Finding」参照)。FAMILY-B自体の正式Closeと、donomana全体のアクセシビリティ課題の解消は別軸であることに注意。

### TIER2-F2 Parent状態

`TIER2-F2`配下4 Finding全てがTECHNICALLY RESOLVEDとなったため、Parent `TIER2-F2` = **✅ TECHNICALLY RESOLVED / PRODUCTION REFLECTED / CLOSED**として扱える(global-fix-triage-1.mdを更新)。

### FAMILY-B Parent状態

FAMILY-B(Focus Trap)は正規化された18 Finding全てがTECHNICALLY RESOLVEDとなったため、**FAMILY-B(Focus Trap欠如という観点)としては全件解消**。ただし、FAMILY-B調査の過程で新規発見・記録された以下のSeparate Findingが未解決のまま残存しており、これらはFAMILY-B自体の直接スコープではないため、FAMILY-Bの解消とは独立してオーナー判断・別Phaseでの対応が必要:

### 継続するSeparate Finding一覧(今回未解決、変更なし)

- **panSet(mogura-tataki)内のdisabled-state設計不整合**(`dwT`/`togCur`/`dwTol`): 既存taxonomy(FAMILY-A/B/C/D/E/I)に合致せず、UNCLASSIFIED/NEEDS OWNER DECISION。
- **cup_game settingsOverlayのVisible Focus問題**(`toggleDwell`等5トグルに`:focus-visible`スタイル皆無): 今回新規発見、UNCLASSIFIED/NEEDS OWNER DECISION。
- **mogura-tataki `moguraTrapFocusables()`の理論上の潜在的設計脆弱性**(`tabindex="-1"`要素混入問題、実害なし): 申し送り事項として記録のみ、Separate Finding化はしない。
- `scrStart`/`scrResult`(mogura-tataki)のFocus Restoration欠如(FAMILY-D候補)。
- Initial Focus欠如(FAMILY-C、mogura-tataki各modal・cup_game settingsOverlay)。
- `panSet`背景クリック閉じ経路のFocus Restorationなし(既存挙動)。
- cup_game settingsOverlayのrole/aria-modal欠如、Escape欠如(既存、別Finding)。
- hiragana-learn/katakana-app traceSampleViewerのaria-modal欠如(既存、別Finding)。
