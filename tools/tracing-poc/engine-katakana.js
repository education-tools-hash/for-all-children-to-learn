...
(function (root) {
  ...
  // against the TARGET's own reference (using its own best permutation
  // search, exactly like relativeCharacterDiscrimination does against
  // other characters) AND against horizontally/vertically mirrored
  // versions of the SAME target reference (each stroke mirrored about its
  // own bbox center, matching the W5 test-generator convention used
  // throughout T2/T3). If the user is meaningfully closer to a mirrored
  // version than to the original, this is a self-reflection (mirror)
  // attempt, independent of whatever assignment was chosen for
  // Position/Completion purposes.
  // ---------------------------------------------------------------------
  function mirrorHorizontalPts(points) {
    let minX = Infinity, maxX = -Infinity;
    points.forEach((p) => { if (p.x < minX) minX = p.x; if (p.x > maxX) maxX = p.x; });
    const cx = (minX + maxX) / 2;
    return points.map((p) => ({ x: 2 * cx - p.x, y: p.y }));
  }
  function mirrorVerticalPts(points) {
    let minY = Infinity, maxY = -Infinity;
    points.forEach((p) => { if (p.y < minY) minY = p.y; if (p.y > maxY) maxY = p.y; });
    const cy = (minY + maxY) / 2;
    return points.map((p) => ({ x: p.x, y: 2 * cy - p.y }));
  }
  const mirrorFeatureCache = new Map();
  function cachedMirroredIntrinsicStrokes(strokeDefs, mirrorFn, tag) {
    const key = tag + '|' + strokeDefs.map((s) => s.d).join('|');
    let cached = mirrorFeatureCache.get(key);
    if (!cached) {
      cached = strokeDefs.map((s) => intrinsicNormalize(mirrorFn(sampleReferencePath(s.d, 64).points)));
      mirrorFeatureCache.set(key, cached);
    }
    return cached;
  }
  function avgDtwToFeatures(userIntrinsicStrokes, candFeatures) {
    const n = userIntrinsicStrokes.length;
    if (candFeatures.length !== n) return Infinity;
    const costMatrix = userIntrinsicStrokes.map((u) => candFeatures.map((c) => structuralDistance(u, c)));
    const idx = candFeatures.map((_, i) => i);
    let best = Infinity;
    permutations(idx).forEach((perm) => {
  ...
  function selfReflectionDiscrimination(userIntrinsicStrokes, refStrokeDefs) {
    const n = userIntrinsicStrokes.length;
    if (refStrokeDefs.length !== n) return null;
    const origFeatures = cachedIntrinsicStrokes(refStrokeDefs);
    const hFeatures = cachedMirroredIntrinsicStrokes(refStrokeDefs, mirrorHorizontalPts, 'h');
    const vFeatures = cachedMirroredIntrinsicStrokes(refStrokeDefs, mirrorVerticalPts, 'v');
    const origAvg = avgDtwToFeatures(userIntrinsicStrokes, origFeatures);
    const hAvg = avgDtwToFeatures(userIntrinsicStrokes, hFeatures);
    const vAvg = avgDtwToFeatures(userIntrinsicStrokes, vFeatures);
    const bestMirrorAvg = Math.min(hAvg, vAvg);
    const bestMirrorType = hAvg <= vAvg ? 'horizontal' : 'vertical';
    const margin = origAvg - bestMirrorAvg;
    return {
      pass: margin < THRESHOLDS.SELF_REFLECTION_MARGIN,
      origAvg, hAvg, vAvg, bestMirrorAvg, bestMirrorType, margin,
    };
  }
  ...
  function evaluateCharacter(userStrokes, referenceStrokeDefs, opts) {
    ...
    // Phase T5-B': Tracing Judgment Level (Metrics Engine / Judgment Policy分離)。
    // opts.judgmentProfileが渡された場合のみAdjustable(motor-tolerance)な4値を上書きする。
    // Multi-Hypothesis Assignment Safety Guard・Self-Reflection Discrimination等の
    // exploit-safety系は対象外(THRESHOLDS固定のまま)。opts.judgmentProfile省略時は
    // Production(precise)と完全に一致する(golden-tests-katakana-independent46.jsの
    // 既存呼び出しに影響なし)。katakana-app.htmlのインラインコピーと同一内容(手作業で同期)。
    ...
    // Phase T3-C'' Multi-Hypothesis Assignment (Section 6-7): when the
    // shape-cost best/second-best permutation gap is below
    // ASSIGNMENT_AMBIGUITY_WINDOW (calibrated across all 46 characters —
    // see analyze-ambiguity-distribution.js / docs Revision 17), the
    // assignment-DEPENDENT guards (minLength, Per-Stroke Quality Floor,
    // Position Guard, Completion Guard) are evaluated against EVERY
    // near-tie permutation, not just the single shape-cost-optimal one.
    // This is safe ONLY because Mirror Self-Confusion is now caught by
    // the assignment-INDEPENDENT Self-Reflection Discrimination guard
    // below (computed once, regardless of which hypothesis wins) —
    // unlike T3-C's Candidate C/D and T3-C''s Candidate E, which tried to
    // improve assignment accuracy itself and thereby removed the
    // accidental Position-Guard-via-wrong-assignment mirror defense
    // without replacing it with anything.
    const n = userFeatures.length;
    function evaluateForAssignment(candAssignment) {
      const minLengthOk = userFeatures.every((uf, i) => {
        const rf = refFeatures[candAssignment[i]];
        return rf.length > 0 ? uf.length / rf.length >= THRESHOLDS.MIN_LENGTH_RATIO : true;
      });
      const strokeResults = userFeatures.map((uf, i) => {
        const j = candAssignment[i];
        ...
        return {
          matchedReferenceStroke: j, referenceLabel: rf.label, shape: shapeScore, coverage, offPath, startEnd,
          direction: dir.component, directionLabel: dir.label, lengthRatio, structuralDistance: structural,
          positionMetric, completion, cost: costMatrix[i][j],
        };
      });
      ...
      const orderPenalty = THRESHOLDS.ORDER_PENALTY_WEIGHT * orderPenaltyFor(candAssignment);
      const spatialScore = spatialAgreement(userFeatures, refFeatures, candAssignment);
      ...
      const perStrokeScores = strokeResults.map((r) => {
        ...
        return clamp01(w.shape * r.shape + w.coverage * r.coverage - w.offPath * offPathPenalty + w.startEnd * r.startEnd + w.direction * r.direction);
      });
      ...
      const assignmentDependentOk = minLengthOk && perStrokeFloorOk && positionGuardOk && completionGuardOk;
      return { assignment: candAssignment, minLengthOk, perStrokeFloorOk, positionGuardOk, completionGuardOk, assignmentDependentOk, strokeResults, orderPenalty, spatialScore, score, avgStrokeScore };
    }
    ...
    if (n >= 2) {
      ...
      const allPerms = permutations(idxAll)
        .map((perm) => { let c = 0; for (let i = 0; i < n; i++) c += costMatrix[i][perm[i]]; return { perm, cost: c }; })
        .sort((a, b) => a.cost - b.cost);
      const gap = allPerms.length > 1 ? allPerms[1].cost - allPerms[0].cost : Infinity;
      if (gap < THRESHOLDS.ASSIGNMENT_AMBIGUITY_WINDOW && !chosen.assignmentDependentOk) {
    ...
    hardGate.minLength = chosen.minLengthOk;
    const hardGatePassed = hardGate.strokeCount && hardGate.minLength && hardGate.grossLocation && hardGate.absoluteGeometry;

    // Relative Character Discrimination (Phase T2-C'''): assignment-
    // independent (does its own internal permutation search per
    // candidate character).
    ...
    // Self-Reflection Discrimination (Phase T3-C''): assignment-
    // independent. Does NOT use `chosen.assignment` at all — compares the
    // whole user trace against the target's own reference vs its own
    // mirrored (H/V) versions, via an independent best-permutation search.
    const selfReflection = selfReflectionDiscrimination(
      userFeatures.map((f) => f.intrinsicPoints),
      referenceStrokeDefs
    );
    const selfReflectionOk = selfReflection ? selfReflection.pass : true;

    hardGate.strokeQualityFloor = chosen.perStrokeFloorOk;
    hardGate.strokePosition = chosen.positionGuardOk;
    hardGate.strokeCompletion = chosen.completionGuardOk;
    hardGate.characterDiscrimination = characterDiscriminationOk;
    hardGate.characterDiscriminationDetail = relativeDiscrimination;
    hardGate.selfReflection = selfReflectionOk;
    hardGate.selfReflectionDetail = selfReflection;

    const pass = hardGatePassed && chosen.assignmentDependentOk
      && characterDiscriminationOk && selfReflectionOk && chosen.score >= T.PASS_THRESHOLD;

    let reason = 'ok';
    if (!hardGatePassed) reason = 'hard_gate_failed';
    else if (!chosen.perStrokeFloorOk) reason = 'stroke_quality_floor_failed';
    else if (!chosen.positionGuardOk) reason = 'stroke_position_failed';
    else if (!chosen.completionGuardOk) reason = 'stroke_completion_failed';
    else if (!characterDiscriminationOk) reason = 'character_discrimination_failed';
    else if (!selfReflectionOk) reason = 'self_reflection_failed';
    else if (chosen.score < T.PASS_THRESHOLD) reason = 'low_score';
    ...
    if (T.BEGINNER_COARSE && !pass) {
      ...
      // レ等でW2attack素通りバグとして発見・修正)。
      const relativePositionOk = chosen.strokeResults.length === 1
        ? chosen.strokeResults[0].positionMetric <= T.STROKE_POSITION_CATASTROPHIC
        : chosen.strokeResults.every((r, i) => {
            const rf = refFeatures[chosen.assignment[i]];
      ...
      const coarseHardSafety = hardGate.strokeCount && hardGate.minLength && absGeom.scaleOk
        && characterDiscriminationOk && selfReflectionOk && totalLengthRatio >= T.COARSE_TOTAL_LENGTH_MIN && relativePositionOk;
      if (coarseHardSafety) {
        const alignShiftX = refOverallCentroid.x - userOverallCentroid.x;
        const alignShiftY = refOverallCentroid.y - userOverallCentroid.y;
        const coarseStrokeResults = chosen.strokeResults.map((r, i) => {
          const rf = refFeatures[chosen.assignment[i]];
          const uf = userFeatures[i];
        ...
        coarsePass = coarseFloorOk && coarseCompletionOk && chosen.avgStrokeScore >= T.COARSE_PASS_THRESHOLD;
        coarseDetail = { totalLengthRatio, relativePositionOk, coarseFloorOk, coarseCompletionOk, coarseTolerance, avgStrokeScore: chosen.avgStrokeScore };
      }
    }
    ...
    return {
      ...
      strokes: chosen.strokeResults,
      orderPenalty: chosen.orderPenalty,
      spatialScore: chosen.spatialScore,
