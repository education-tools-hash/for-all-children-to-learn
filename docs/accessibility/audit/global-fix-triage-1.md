...
|---|---|---|
| ✅ Closed | TIER1-F1 | 1 |
| ✅ Technically Resolved | TIER2-F6 | 1 |
| 🟡 Partially Resolved | TIER2-F3(cup_game分のみ解消) | 1 |
| 🔴 Open(未着手) | TIER1-F2,F3,F4,F5,F6 / TIER2-F1,F2,F4,F5 / TIER3-F1,F2,F3 | 12 |

**解消済みFindingをFix backlogへ再度入れない**(§5準拠)。TIER2-F3はcup_game分を再修正対象にせず、ongaku-app分のみを以降のFAMILY-B/Cの対象として扱う。
