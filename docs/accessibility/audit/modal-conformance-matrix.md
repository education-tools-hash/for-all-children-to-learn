...
| matching-app | ✅ | ✅ | ✅(Pattern1) | ✅(端点循環) | ✅**Fix済み(RC commit 6e9ddb6、Production未反映)。how-ov/settings-ov/record-ovの3モーダルで確認。vs-result-ov/clear-ov/edit-ovの3モーダルはtitleにtabindex="-1"・初期focus自体が無く別問題(本Batch対象外、記録のみ)** | ✅ **参照実装** | ✅ | ✅ | ✅(系統A) **参照実装** | A11yパネル例外・背景抑制は引き続き参照実装水準 |
...
| okane-app | ✅ | ✅ | ✅(Pattern1) | ✅(端点循環) | ✅**Fix済み(RC commit 6e9ddb6、Production未反映)。help/settings/records/customの4モーダル共通ハンドラで一括修正済み** | ✅ **参照実装** | ✅ | 🟡**訂正: recordsModalのみ実装(`recordsModalOpener`パターン)。help/settings/customの3モーダルはFocus Restoration自体が未実装でBODY退行することをWCAG-JIS-FIX-FAMILY-J-1で新規発見(FAMILY-Jとは別問題、本Batch対象外、記録のみ)** | ✅(系統A) **参照実装** | A11yパネル例外・背景抑制は引き続き参照実装水準 |
...
| schedule-app | ✅ | ✅ | ✅(Pattern1) | ✅(端点循環) | ✅**Fix済み(RC commit 6e9ddb6、Production未反映)。print-modal/img-modalの2モーダルで確認(new-modalは初期focus自体が無く別問題、対象外)。独自の防御分岐[`modal.contains(active)`]は本失敗モードには無効だったが、境界修正で解消** | ✅ **参照実装** | 🟡**print-modalはFocus Restoration未実装(BODY退行)をWCAG-JIS-FIX-FAMILY-J-1で新規発見(別問題、記録のみ)** | — (未検証) | 🟡(系統C、inertなし) | 背景inertなしでも端点循環が機能する例 |
...
| gaze-keyboard(settingsModal) | ✅ | ✅ | ✅(Pattern1、PILOT-F1で修正済み) | ✅(端点循環) | ✅**Fix済み(RC commit 6e9ddb6、Production未反映)** | ✅(HARDEN-3で修正済み) | ✅ | ✅ | ✅(系統A) | Fix実績最多。Gaze/dwellコードへの副作用なしを確認 |
...
| **Reverse Tab境界(v1.1新設)** | **7(register-app・shiritori2・bosai-app・ongaku-app[modal-help]・time-timer・tokei-app・janken-app、いずれも構造的に非該当と実機確認済み)+ongaku-app(pin/export/share、Production反映済み)+matching-app/okane-app/schedule-app/gaze-keyboard(settingsModal)(RC Fix済み、Production未反映)=12** | 0 | 0(WCAG-JIS-FIX-FAMILY-J-1で全件解消。詳細は下記) | 6(Trap自体がないため非該当のアプリ) |
...
**[2026-09-07更新]** WCAG-JIS-FIX-FAMILY-J-1でmatching-app・okane-app・schedule-app・gaze-keyboard(settingsModal)のReverse Tab境界をRC修正済み(commit `6e9ddb6`、branch `fix/family-j-reverse-tab-boundary`、**Production未反映**)。time-timer・tokei-app・janken-appは実機再確認の結果、Sub-pattern α等により構造的に非該当と判明し、CONFIRMED PASSへ訂正(修正不要)。**Contract全項目(Reverse Tab境界含む)でPASS/RC-PASSしているのは、Production反映済みのongaku-app(modal-pin/export/share)に加え、RC段階でmatching-app・okane-app・schedule-app・gaze-keyboard(settingsModal)も達成。** 18アプリ中17アプリが何らかのContract項目でFAIL/PARTIALを持つ状況自体は変わらないが、FAMILY-J単体では対象アプリ全ての解消方針が確定した。
...
| **FAMILY-J** | **Reverse Tab境界** | **0(全対象解消。ongaku-app分Production反映済み、matching-app/okane-app/schedule-app/gaze-keyboard[settingsModal]分RC Fix済み[Production未反映]、time-timer/tokei-app/janken-appはCONFIRMED PASSで対象外に訂正)** |
...
| 5(v1.1追加) | matching-app・okane-app・schedule-app・gaze-keyboard(settingsModal) | Reverse Tab境界(FAMILY-J) | 実機再現済み。initial focus直後の最初のShift+Tabでmodal外へfocusが漏れる | ✅**RC Fix済み(commit `6e9ddb6`、WCAG-JIS-FIX-FAMILY-J-1)。Production未反映**。境界判定に`initialFocusAnchor`(`aria-labelledby`参照先)を追加、ongaku-appのFix実装[commit 891fd90]と同一パターン |
