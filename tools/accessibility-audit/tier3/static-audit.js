// WCAG-JIS-AUDIT-1-TIER3: Tier3 6アプリの静的構造監査(Structure + Modal/Focus Trapパターン)。
// Read-only。Productionファイルは一切変更しない。
const fs = require('fs');
const path = require('path');
const ROOT = path.resolve(__dirname, '..', '..', '..');

const TIER3 = ['nazori-app','timetable-app','yomikaki-app','sugoroku-app','sst-app','slideshow-sakusei'];
...
for (const app of TIER3) {
  ...
  // buttons with no accessible name candidate (no text content marker, no aria-label/aria-labelledby) - screening only
  r.buttonsWithAriaLabel = (html.match(/<button[^>]*aria-label=/g) || []).length;
  r.buttonsTotal = (html.match(/<button[\s>]/g) || []).length;
...
for (const [app, r] of Object.entries(results)) {
  console.log(app.padEnd(20),
    ...
    'buttons=' + r.buttonsTotal + '(aria-label=' + r.buttonsWithAriaLabel + ')'
  );
}
