...
| matching-app | ✅ | ✅ | ✅(Pattern1) | ✅(端点循環) | ✅**Fix済み・Production反映済み(commit 6e9ddb6、main HEAD `64e026b`、WCAG-JIS-FIX-FAMILY-J-1-RELEASE)。how-ov/settings-ov/record-ovの3モーダルで確認。vs-result-ov/clear-ov/edit-ovの3モーダルはtitleにtabindex="-1"・初期focus自体が無く別問題(本Batch対象外、記録のみ)** | ✅ **参照実装** | ✅ | ✅ | ✅(系統A) **参照実装** | A11yパネル例外・背景抑制は引き続き参照実装水準 |
