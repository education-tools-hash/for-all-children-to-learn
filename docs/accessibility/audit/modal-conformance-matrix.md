...
|---|---|---|---|---|---|---|---|---|---|---|
| register-app | ✅ | ✅ | 🟡(Pattern3、最初のcontrol) | ✅(端点循環、**product-modalのみ**。delete-modalはFocus Trap自体が実装されておらずShift+TabでモーダルOK外(背景`record-open-btn`等)へ抜けることを実機確認、既存の別Finding・FAMILY-B領域・今回Fix対象外) | ✅(Pattern3のためinitial focus=firstと一致、構造的に非該当) | ❌(TIER1-F2) | ✅ | ✅product-modal(TIER1-F5)・delete-modal(TIER1-F5B)ともTECHNICALLY RESOLVED / PRODUCTION REFLECTED(WCAG-JIS-FIX-FAMILY-D-REGISTER-DELETE-1-RELEASE、commit `3282b02`、Production Validation PASS)。delete-modalは全close経路(cancel/Escape/confirm削除)でBODY退行していたが解消。`dmDeletingId`+`dmDeletingIndex`を保持し、cancel/Escapeは同一`.product-card`へ、confirm(実削除)後は再描画後の同一index位置のカード(繰り上がった次カード、無ければ最後のカード、グリッドが空なら`.add-product-card`)へlogical replacement。Production ValidationでLEVEL-A実機確認済み(境界ケース: 最後のカード削除・全削除も確認) | 🟡(系統B、モーダル自身のみ) | 8モーダル共通ヘルパー、Fix影響範囲大。詳細はfamily-d-focus-restoration-close-1.md参照 |
...
| mogura-tataki | 🟡(aria-label直接、labelledby未使用) | 🟡 | ❌(なし) | ❌(TIER1-F3)。**[2026-09-08訂正] `WCAG-JIS-FAMILY-B-LEDGER-NORMALIZE-1`でコード確認した結果、`scrStart`・`scrResult`・`panSet`・`panRec`・`panHow`という5つの独立した`role="dialog" aria-modal="true"`要素が存在し、いずれもTab keydown処理を持たない(Pattern B1)ことが判明。旧「screen/panelベースの独自アーキテクチャ、深掘り要」は解消し、他アプリと同型のB1として扱う(LEVEL-B、実機未検証)** | — N/A(Trap自体なし) | — | — (未検証) | — (未検証) | ❌(系統D) | 詳細はfamily-b-focus-trap-finding-ledger.md参照 |
...
| tyushi | ✅(help-overlay)。**settings-panelは`aria-modal`無し・backdrop無しの非modal disclosure panelと判定(WCAG-JIS-FAMILY-D-TYUSHI-DESIGN-1)、本列はhelp-overlayの評価** | ✅(aria-label、TIER1-F1対応時に確認済み) | ❌(なし、help-overlay) | ❌(TIER1-F3、help-overlay。settings-panelはFocus Trap意図的不在=非modalとして正常) | — N/A(Trap自体なし) | — | ✅(help-overlayのみ。settings-panel自体にはEscape未実装、別Finding扱い) | ✅**NOT APPLICABLE(WCAG-JIS-FAMILY-D-TYUSHI-DESIGN-1で確定)**: settings-panelは非modal disclosure panel(Tabで背景へ自然に抜ける、背景クリック常時可能、Gaze/dwell設定を主UIを見ながら並行調整する設計上の要請)と判定、Modal Focus Restoration Contract対象外。派生Finding(A11yパネルProxy再トグルclose時の非表示要素focus残留、P3)は別Backlogへ記録、本Fix対象外 | ❌(系統D) | H1修正済み(AUDIT-35-H1-IMPL-2)。settings-panel詳細はfamily-d-tyushi-design-1.md参照 |
...
| cup_game(settingsOverlay) | 🟡(`role`/`aria-modal`未設定、`class="modal-overlay"`のみ。FAMILY-D対象外の別Finding、記録のみ) | — (未検証) | — (未検証、`donomanaFirstFocusable`によるA11yパネルProxy側fallbackで`dwellTimeSlider`等へ着地する非理想パターン。FAMILY-C領域の別Finding、記録のみ) | ❌(Trap自体なし、既存のまま) | — N/A(Trap自体なし) | — | ❌(Escapeで閉じる仕組み自体が存在しない。既存の別Finding、記録のみ、本Fix対象外) | ✅**TECHNICALLY RESOLVED / PRODUCTION REFLECTED(WCAG-JIS-FIX-FAMILY-D-BATCH-4-RELEASE、commit `1170a28`、Production Validation PASS)**: close button・openHelp()による相互排他close(2経路のみ、他に背景クリック等の経路なし)いずれもBODY退行が発生していたが解消。実trigger(`#gearBtn`)は共通A11yパネルProxy構造によりopacity:0/tabindex=-1化されているため、okane-app settingsModal・matching-app settings-ovと同型の判断で可視の`donomanaA11yBtn`へ固定復帰。Production ValidationでLEVEL-A実機確認済み | ❌(系統D) | role/aria-modal欠如・Escape欠如・Initial Focus非理想は別Finding(FAMILY-A/C寄り)として記録、本Fix非対象。詳細はfamily-d-cross-app-audit-1.md参照 |
...
| Focus Trap(Forward) | **12行**(register-app[product-modalのみ]・matching-app・time-timer・okane-app・schedule-app・janken-app・tokei-app・gaze-keyboard[settingsModal]・shiritori2・bosai-app・ongaku-app[modal-help]・ongaku-app[pin/export/share]) | 0 | **10行/正規化後18 Finding(modal単位)**。**[2026-09-08、WCAG-JIS-FAMILY-B-LEDGER-NORMALIZE-1で正規化]** 本表は行(app/画面)単位の集計であり、nazorin-print(batchModal・libModalの2要素)・gaze-keyboard[profileModal/hrModal](2要素)・mogura-tataki(scrStart/scrResult/panSet/panRec/panHowの5要素)のように1行に複数modalを含む場合がある。**modal単位の正確な件数(18件)は`family-b-focus-trap-finding-ledger.md`を正式なSource of Truthとする。** 行内訳: mogura-tataki・scratch-app・nazorin-print・tyushi[help-overlay]・gaze-keyboard[profileModal/hrModal]・hiragana-learn・katakana-app・cup_game[helpModal]・cup_game[settingsOverlay]・register-app[delete-modal] | **1**(tyushi[settings-panel]、非modal確定によりN/A) |
...
| Focus Restoration | **14**(matching-app・okane-app・nazorin-print・schedule-app・scratch-app・cup_game[settingsOverlay]・gaze-keyboard[settingsModal]・hiragana-learn・katakana-app・shiritori2・bosai-app・cup_game[helpModal]・ongaku-app[modal-help]・ongaku-app[pin/export/share]) | 0 | 1(register-app、TIER1-F5) | **6**(time-timer・mogura-tataki・janken-app・tokei-app・gaze-keyboard[profileModal/hrModal]は未検証。**tyushi(settings-panel)はWCAG-JIS-FAMILY-D-TYUSHI-DESIGN-1でNOT APPLICABLEと確定済み**、未検証ではなく判定済みN/A) |
...
| FAMILY-B | Focus Trap(Forward) | **18 Finding(modal単位、2026-09-08、WCAG-JIS-FAMILY-B-LEDGER-NORMALIZE-1で正規化・確定)。[訂正履歴] WCAG-JIS-FAMILY-B-CROSS-APP-AUDIT-1では行(app/画面)単位で10行と報告し、かつ最終報告文中で「CONFIRMED FAIL=12」とPattern内訳「B1=10+B7=1=11」が相互に矛盾していた。原因はnazorin-print(batchModal/libModalを1行に束ねていた)・gaze-keyboard(profileModal/hrModalを1行に束ねていた)・mogura-tataki(scrStart/scrResult/panSet/panRec/panHowの5要素を1行の曖昧な記述に隠していた)という「1行=複数Finding」の集計単位の不統一と、地の文の単純な算術誤記であった。「1 Finding = 1 Ledger Row」原則で全18件を個別Finding ID化し、`family-b-focus-trap-finding-ledger.md`を正式なSource of Truthとする(Evidence Level: LEVEL-A 6 + LEVEL-B 12 = 18、Pattern: B1 18 + その他0 = 18、いずれも内部整合済み)。ongaku-app pin/export/share分はFix済みのため対象外。tyushi[settings-panel]は非modal確定によりN/A(18件に含まず)** |
